...
// TODO: Update this when repository is created
const GITHUB_API_URL: &str = "https://api.github.com/repos/your-org/ai-image-decoder/releases/latest";
    ...
    match std::fs::read_to_string("VERSION") {
        ...
        Err(_) => {
            match std::fs::read_to_string("../VERSION") {
                Ok(v) => v.trim().to_string(),
                Err(_) => {
                    std::env::var("AI_IMAGE_DECODER_VERSION")
                        .unwrap_or_else(|_| "0.1.0-alpha".to_string())
                }
            }
        }
    }
...
fn is_newer_version(current: &str, latest: &str) -> bool {
    ...
    // Handle pre-release versions (e.g., "0.1.0-alpha")
    ...
    // If base versions are equal, check if current is pre-release
    if current.contains('-') && !latest.contains('-') {
        return true; // Pre-release is older than release
    }
